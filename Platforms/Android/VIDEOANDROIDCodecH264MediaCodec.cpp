  ...
  if(!initialized)
    {
      ...
      if(status != AMEDIA_OK)
      {
        XTRACE_PRINTCOLOR(4, __L("Error Configuring MediaCodec: "));

        switch(status)
        {
        case AMEDIA_ERROR_UNKNOWN:
          XTRACE_PRINTCOLOR(4, __L("UNKNOWN"));
          break;
        case AMEDIA_ERROR_MALFORMED:
          XTRACE_PRINTCOLOR(4, __L("MALFORMED"));
          break;
        case AMEDIA_ERROR_UNSUPPORTED:
          XTRACE_PRINTCOLOR(4, __L("UNSUPPORTED"));
          break;
        case AMEDIA_ERROR_INVALID_OBJECT:
          XTRACE_PRINTCOLOR(4, __L("INVALID OBJECT"));
          break;
        case AMEDIA_ERROR_INVALID_PARAMETER:
          XTRACE_PRINTCOLOR(4, __L("INVALID PARAMETER"));
          break;
        }
      }
      ...
      if(status != AMEDIA_OK)
      {
        XTRACE_PRINTCOLOR(4, __L("Error Starting Codec"));
      ...
      if(framereader->ReadFrame(frame)) // make sure we can read the frame
        {
          ...
          if(idx < 0)
            {
              XTRACE_PRINTCOLOR(4, __L("Android MediaCodec: No valid input buffer ID found"));
              return;
            }
          ...
          if(!buf)
            {
              XTRACE_PRINTCOLOR(4, __L("Android MediaCodec: No valid buffer found"));
              return;
            }
          ...
          if(size < frame.GetSize())
            {
              XTRACE_PRINTCOLOR(4, __L("Android MediaCodec: Error, input buffer is smaller than read frame"));
            }
          else
            {
          ...
          XTRACE_PRINTCOLOR(2, __L("InputBuffer Queued Back"));
        }
